% FILENAME=plot-fluorescence-corrected.m
...
title (horzcat('Fluorescence (mg Chl/m^3) along 172\circW, ',sday,'-',eday));
...
title (horzcat('Fluorescence (mg Chl/m^3) along 172\circW, ','04/26/98','-','04/30/98'));
clabel(c,h,'color','w') %make labels white
...
title (horzcat('Fluorescence (mg Chl/m^3) along 158\circW, ',sday,'-',eday));
clabel(c,h,'color','w') %make labels white
...
colorbar('vertical')
hold on;
v=(-0.09:0.2:1.0);% c=contour(x,z,T,v,'w');
[c,h]=contour(x2,y2,z2,v,'white');
xlabel ('Latitude (deg N)');
ylabel ('Pressure (dbars)');
